    ...
    if request.method == "POST": 
        ...
        if password == c_password: 
            ...
            if email.upper() not in email_data_list: 
                ...
                return render_template('login.html', message="Successfully Registered! Please go to 
login section") 
...
def add_recipie(): 
    if request.method == "POST": 
        ...
        nutrients = request.files['nutrients'] 
        ...
        query = "INSERT INTO recipies(`name`, `image`, `nutrients`) VALUES (%s, %s, %s)" 
        values = (r_name, mypath, nutrients) 
        executionquery(query, values) 
...
def update_recipie(): 
    ...
    query = "UPDATE recipies SET name = %s, image = %s, calaries = %s WHERE id = %s" 
    values = (r_name, mypath, nutrients, id) 
    executionquery(query, values) 
 
    query = "SELECT * FROM recipies" 
    data = retrivequery2(query) 
    return render_template('view_recipie.html', datas = data, message = "Successfully
                           Updated!") 
